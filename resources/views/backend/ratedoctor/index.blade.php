...
@section('main-content')
 ...
 <div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary float-left">Danh sách đánh giá bác sĩ</h6>
    </div>
    <div class="card-body">
      <div class="table-responsive">
        @if(count($reviews)>0)
        <table class="table table-bordered table-hover" id="doctor-review-dataTable" width="100%" cellspacing="0">
          <thead>
            <tr>
              <th>#</th>
              <th>Đánh giá bởi</th>
          ...
          <tbody>
            @foreach($reviews as $review)
                <tr>
                    ...
                    <td>
                        <a href="{{route('doctor-review.edit', $review->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="Chỉnh sửa" data-placement="bottom"><i class="fas fa-edit"></i></a>
                        <form method="POST" action="{{route('doctor-review.destroy',[$review->id])}}">
                          @csrf
                          @method('delete')
...
@push('scripts')
  ...
  <script>
      $('#doctor-review-dataTable').DataTable( {
            "columnDefs":[
                {
                    "orderable":false,
                    "targets":[5,6]

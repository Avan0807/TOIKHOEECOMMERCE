                            ...
                            <form action="{{ route('cart.update') }}" method="POST">
                                ...
                                @if(count($cartItems) > 0)
                                    @foreach($cartItems as $key => $cart)
                                        @php
                                            // Kiểm tra nếu product tồn tại và có dữ liệu
                                            $product = $cart->product ?? null;
                                            $photo = isset($product['photo']) ? explode(',', $product['photo']) : ['default-image.jpg'];
                                            $title = $product['title'] ?? 'Sản phẩm không có tiêu đề';
                                            $slug = $product['slug'] ?? '#';
                                            $summary = $cart['summary'] ?? 'Không có mô tả';
                                            $price = number_format($cart['price'] ?? 0, 0, ',', '.');
                                            $amount = number_format($cart['amount'] ?? 0, 0, ',', '.');
                                            $quantity = $cart->quantity ?? 1;
                                        @endphp
                                        <tr>
                                            <td class="image" data-title="Số"><img src="{{ $photo[0] }}" alt="Hình ảnh sản phẩm"></td>
                                            ...
                                            <td class="total-amount cart_single_price" data-title="Tổng"><span class="money">{{ $price }}đ</span></td>
                                            <td class="qty" data-title="Số lượng">
                                                <div class="input-group">
                                                    ...
                                                    <input type="text" name="quant[{{ $key }}]" class="input-number" data-min="1" data-max="100" value="{{ $quantity }}">
                                            ...
                                            <td class="price" data-title="Giá"><span>{{ $amount }}đ</span></td>
